...
object MakeGeometries
    extends CommandApp(
      ...
      main = {
        ...
        (orcO, outO, numPartitionsO, cacheDirO).mapN {
          (orc, out, numPartitions, cacheDir) =>
...
//        .set("spark.sql.orc.filterPushdown", "true")
            ...
            val nodeGeoms = cache
              .orc("node_geoms", ProcessOSM.NodeGeomSchema) {
                ProcessOSM.constructPointGeometries(nodes)
              }
              .withColumn("minorVersion", lit(0))

            val wayGeoms = cache.orc("way_geoms", ProcessOSM.WayGeomSchema) {
              ProcessOSM.reconstructWayGeometries(df, nodes)
            }

            val relationGeoms = cache.orc("relation_geoms", ProcessOSM.RelationGeomSchema) {
